...
// static void f_Derivative_IIR_Filter(PID_t *pid);
    ...
    uint8_t improve)
{
    ...
    pid->IMaxOut = imax_out;
    pid->IMinOut =imin_out;
    pid->MaxOut = max_out;
    pid->MinOut = min_out;
    pid->Ref = 0;
    ...
    // float num[3] = {0.0001198f, 0.0002396f, 0.0001198f};
    // float den[3] = {1.0f,      -1.9868f,    0.9869f};
    // uint8_t order = 3;
    // IIR_Filter_Init(pid->derivativeIIR, num, den, order);
...
float Inc_PID_Calculate(PID_t *pid,float measure,float ref)
{
    ...
    if (!isnormal(pid->Ref))
        pid->Ref = 0;
    pid->Err0 = pid->Ref - pid->Measure;
    
    if (abs(pid->Err0) > pid->DeadBand)
    {
        pid->Pout = pid->Kp * (pid->Err0 - pid->Err1);
        pid->Iout = pid->Ki * pid->Err0 * pid->dt;
        pid->Dout = pid->Kd * (pid->Err0 - 2*pid->Err1 + pid->Err2) / pid->dt;
...
static void f_Integral_Limit(PID_t *pid)
{
    ...
    if (temp_Iout > pid->IMaxOut)
    {
        pid->ITerm = 0;
        pid->Iout = pid->IMaxOut;
    }
    if (temp_Iout < pid->IMinOut)
    {
        pid->ITerm = 0;
        pid->Iout = pid->IMinOut;
    }
}
...
// static void f_Derivative_IIR_Filter(PID_t *pid)
// {
//     pid->Dout = IIR_Filter_Calculate(pid->derivativeIIR,pid->Dout);
// }
